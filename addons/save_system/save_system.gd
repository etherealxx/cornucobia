...
var default_file_path : String = "user://cornucobia_save.sav"
var encrypted_file_path : String = "user://cornucobia_save_encrypted.sav"
var encryption_key : String = "" #@TODO change this later
var encryption_key_path : String = "res://secrets/encryption_keys/savemadeeasy_encryption_key.gd"
var use_encryption : bool = true
...
func save(file_path : String = default_file_path, enc_file_path := encrypted_file_path):
	# :EditorEmptySavingPermission
	...
	if use_encryption:
		...
		if dir.file_exists(encryption_key_path):
			var save_enc_script = ResourceLoader.load(encryption_key_path)
			var save_enc_node = Node.new()
			save_enc_node.set_script(save_enc_script)
			if save_enc_node.has_method("get_encryption_key"):
				encryption_key = save_enc_node.get_encryption_key()
				save_enc_node.queue_free()
			else:
				push_error("method get_encryption_key not found")
			file = FileAccess.open_encrypted_with_pass(enc_file_path, FileAccess.WRITE, encryption_key)
		else:
...
func _load(file_path : String = default_file_path, enc_file_path := encrypted_file_path):
	var file : FileAccess
	if use_encryption:
		var dir = DirAccess.open("res://")
		if dir.file_exists(encryption_key_path):
			...
			if save_enc_node.has_method("get_encryption_key"):
				...
				push_error("method get_encryption_key not found")
			file = FileAccess.open_encrypted_with_pass(enc_file_path, FileAccess.READ, encryption_key)
		else: # fallback if no encryption
